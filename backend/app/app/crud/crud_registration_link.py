...
from app.crud import CRUDBase, user
from app.models import Registration_Link, User

from app.schemas import RegistrationLinkCreate, RegistrationLinkUpdate
from datetime import datetime, timedelta
from app.core import settings
from app.utils import generate_registration_link_value, send_new_registration_link_email
import logging


class CRUDRegistrationLink(
    CRUDBase[Registration_Link, RegistrationLinkCreate, RegistrationLinkUpdate]
):
    ...
    def validate(
        self, db: Session, value: str, current_user: User
        ...
        if (
            ...
            if curDate < dateExpiration:
                ...
                user.update(db=db, db_obj=current_user, obj_in={"is_verified": True})
        else:
    ...
    def generate_and_insert(self, db: Session, fk_user: int) -> Registration_Link:
        ...
        new_registration_link = self.create(db=db, obj_in=registration_link_tmp)
        ...
        if settings.EMAILS_ENABLED:
            send_new_registration_link_email(
                user_in_db.email, user_in_db.full_name, new_registration_link.value
            )
