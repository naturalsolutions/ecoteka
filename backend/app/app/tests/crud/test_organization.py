...
from app.schemas.organization import OrganizationMode, OrganizationCreate, OrganizationCreateRoot, OrganizationUpdate
...
def test_get_by_id_or_slug(db: TestingSessionLocal, faker) -> None:
    ...
    result = crud_organization.organization.get_by_id_or_slug(db, id=organization.id)

    if result:
        assert result.id == organization.id

    result = crud_organization.organization.get_by_id_or_slug(db, id=organization.slug)
...
def test_get_by_name(db: TestingSessionLocal, faker: Faker) -> None:
    ...
    organization = crud_organization.organization.create(db, obj_in=organization_in)
    founded = crud_organization.organization.get_by_name(db, name=organization.name)
    if founded:
...
def test_get_by_slug(db: TestingSessionLocal, faker: Faker) -> None:
    ...
    organization = crud_organization.organization.create(db, obj_in=organization_in)
    founded = crud_organization.organization.get_by_slug(db, slug=organization.slug)
    if founded:
...
def test_get_by_path(db: TestingSessionLocal, faker: Faker) -> None:
    ...
    organization = crud_organization.organization.create(db, obj_in=organization_in)
    founded = crud_organization.organization.get_by_path(db, path=organization.path)
    if founded:
...
def test_get_teams(db: TestingSessionLocal, faker: Faker) -> None:
    ...
    teams = crud_organization.organization.get_teams(db, parent_id=organization_root.id)
...
def test_get_path(db: TestingSessionLocal, faker: Faker) -> None:
    ...
    organization = crud_organization.organization.create(db, obj_in=organization_in)
    team = crud_organization.organization.create(db, obj_in=OrganizationCreate(
        mode=OrganizationMode.PRIVATE,
        name=faker.name(),
