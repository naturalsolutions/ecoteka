...
const TreeInfosProperties: React.FC<TreeInfosPropertiesProps> = (props) => {
  ...
  const [imagesActiveStep, setImagesActiveStep] = React.useState(0);

  const handleNext = () => {
    setImagesActiveStep((prevActiveStep) => prevActiveStep + 1);
  };
      ...
      {images?.length > 0 && (
        ...
        >
          ...
          <MobileStepper
            ...
            position="static"
            variant="text"
            activeStep={imagesActiveStep}
            nextButton={
              <Button
                size="small"
                onClick={handleNext}
                disabled={imagesActiveStep === images?.length - 1}
              >
                <KeyboardArrowRight />
              </Button>
            }
