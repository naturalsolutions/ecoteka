...
describe("Edit basic tree form", () => {
  beforeEach(function () {
    ...
    cy.fixture("trees").then(function (data) {
      ...
      cy.get("[data-test=tree-basic-form-treeOfInterest] input").as(
        "treeOfInterest"
      );
      this.data = data;
    });
  ...
  it("success empty tree form", function () {
    ...
    cy.get("@plantationDate").should("be.empty");
  });
  ...
  //There is no success modal or message
  ...
  it("fail edit tree", function () {
    cy.get("@canonicalname").type(this.data.failtree.canonicalname);
    cy.get("[data-test=tree-page]").click();
  ...
  //The autocomplete textfield is not sending the good request params when it's only one letter
  it("test autocomplete canonical name tree", function () {
    cy.get("@canonicalname").clear().type("a");
